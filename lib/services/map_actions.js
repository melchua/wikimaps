...
module.exports = function makeMapActions(knex) {
    ...
    function getMaps(){
      return delay()
        .then(() => {
          return knex.select('maps.*')
            .from('maps')
            .returning('*').catch(function (err) {console.log(err)})
            .then((maps) => {
              console.log("Get list of maps from maps table", maps);
              return maps;
            });
        });
    }
    ...
    function delOrCreateFavMapByKey(key, userId){
      return getMapByKey(key)
        .then((map) => {
          // If a map is found by that key, add to favorites
          if(map) {
            return knex('maps')
            .insert({map_key: key, user_id: userId})
            .returning('*')
    ...
    */
    ...
    function createPlaceWithMapKey(mapKey, userId, {name, description, img, lat, lng}){
      ...
      console.log("createplaceWith map key:");
      return mapPromise
        .then((map) => {
          return knex('places')
            .insert({
...
//     function checkFavorites(key){
//       return knex('faverites')
//         .first('*')
//         .where({map_key: key});
//     }

//     function addOrDeleteFavorites(key, userId){
//       return checkFavorites(key)
//         .then((map) => {
//           // If a map is found by that key, return it.
//           if(map) {
...
//     function addDelFavorites(map_key, userId){
//       return addOrDeleteFavorites(map_key, userId)
//       //  .then((map) => {
//           // const addMapFav = {};
//           // if(map_key){
//           //   addMapFav.map_key = map_key;
//           // }
//           // if(userId) {
//           //   addMapFav.user_Id = userId;
//           // }
//           // return knex('favorites')
//           //   .insert(addMapFav)
// //            .where({id: map.id})
