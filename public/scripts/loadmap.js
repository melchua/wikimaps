...
// onclick='saveData(markerForNew)
  ...
  $('#id-of-input').click(function(){
      ...
      // var savedFavTitle = escape(document.getElementById('mapTitle').value);
      var saveFavMapTitle = {
        map_id: req.params.id
      };
      console.log("Favorite TITLE" , saveFavMapTitle);
//want user id and map id saves to favorites table from there can pull up usermap id
    $.ajax({
      url: "/maps/:key/favorites", //NOT SURE SINCE AJAX??
      method: "POST",
      data: saveFavMapTitle,
      // dataType: "json",
      success: (data) => {
        // data = JSON.parse(data);
        console.log('success in mapAjax', data);
